...
impl Allocation {
    ...
    pub const fn chunk_id(&self) -> Option<core::num::NonZeroU64> {
        self.chunk_id
    }
...
impl MemoryBlock {
        ...
        let device_memory = {
            ...
            let alloc_info = match allocation_scheme {
                AllocationScheme::DedicatedBuffer(buffer) => {
                    dedicated_memory_info = vk::MemoryDedicatedAllocateInfo {
                        buffer,
                        ..dedicated_memory_info
                    };
                    alloc_info.next(&mut dedicated_memory_info)
                }
            ...
            device
                ...
                .map_err(|e| match e {
                    vk::Error::OutOfDeviceMemory => AllocationError::OutOfMemory,
                    ...
                    .and_then(|p| {
                        core::ptr::NonNull::new(p).map(SendSyncPtr).ok_or_else(|| {
                            AllocationError::FailedToMap(
                                "Returned mapped pointer is null".to_owned(),
                            )
                        })
                    })
            })
    ...
    fn destroy(self, device: &vk::Device) {
...
impl MemoryType {
    fn allocate(
        ...
        let is_host = self
            .memory_properties
            .contains(vk::MemoryPropertyFlags::HostVisible);
            ...
            if let Some(mem_block) = mem_block {
                ...
                match allocation {
                    Ok((offset, chunk_id)) => {
                        ...
                        {
                            // SAFETY: `offset` is returned by the sub-allocator for this memory
                            // block and is within the mapped block range.
    ...
    fn free(&mut self, allocation: Allocation, device: &vk::Device) -> Result<()> {
...
impl Allocator {
    pub fn new(desc: &AllocatorCreateDesc) -> Result<Self> {
        let mem_props = desc.physical_device.get_memory_properties();
        ...
        let physical_device_properties = desc.physical_device.get_properties();
    ...
    pub fn allocate(&mut self, desc: &AllocationCreateDesc<'_>) -> Result<Allocation> {
        ...
        let mem_loc_preferred_bits = match desc.location {
            MemoryLocation::GpuOnly => vk::MemoryPropertyFlags::DeviceLocal,
