...
const fn align_down(val: u64, alignment: u64) -> u64 {
    val & !(alignment - 1u64)
}
...
const fn is_on_same_page(offset_a: u64, size_a: u64, offset_b: u64, page_size: u64) -> bool {
...
impl SubAllocator for FreeListAllocator {
    ...
    fn free(&mut self, chunk_id: Option<core::num::NonZeroU64>) -> Result<()> {
        ...
        if let Some(next_id) = next_id
            && self.chunks[&next_id].allocation_type == AllocationType::Free
        {
            self.merge_free_chunks(chunk_id, next_id)?;
        }

        if let Some(prev_id) = prev_id
            && self.chunks[&prev_id].allocation_type == AllocationType::Free
        {
