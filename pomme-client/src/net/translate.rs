...
//!
//! Component-era item patches (767-775) are normalized before Azalea's latest
//! decode: outer component ids are remapped semantically, known historical
//! layout changes have exact walkers/transcoders, nested item stacks recurse
//! through the same normalizer, and registry-bearing payloads that cannot be
//! remapped losslessly are consumed and dropped individually rather than
//! reinterpreting stale numeric ids in 26.2 space. Protocol 766 has its own
//! targeted player-inventory/equipment rewrite for the stable scalar/text
//! subset, full vanilla enchantments, Tool removal, and private mining
//! metadata. Tool values on 766-773 degrade to Pomme's native per-item
//! defaults; 774-775 retain their layout-compatible Tool payloads and resolve
//! direct block ids through Pomme's negotiated block registry at gameplay
//! lookup.
//!
//! Known 26.3 limitation (accepted): generic inbound item stacks are not yet
//! normalized through that component walker. Components at/after 26.3's first
//! differing id can therefore misdecode in those paths; nested item stacks
//! keep their source-version ids, and components whose value layout changed
//! (`pot_decorations`, inline `trim`/`instrument`, or `teleport_randomly`)
//! require the targeted 26.3 walkers used by supported packet paths below.
