...
pub struct GameState {
    ...
    /// dropped only when *that* section was edited again ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    /// editing one section never invalidates a sibling section's in-flight
    /// result. Sections meshed together as one edit span share one gen
    /// value.
    ...
    /// ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â movement, not rotation, drives recomputes
    /// (vanilla's cadence).
...
impl GameState {
    ...
    /// ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â vanilla's async, movement-gated cadence). The
    /// walk is rotation-independent; frustum culling runs per-frame on the
    /// GPU.
    ...
    /// render distance meshes regardless of visibility ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
    /// occlusion gates only drawing ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and the queue orders
...
/// contiguous) visible set enqueues as a few range jobs ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â one
/// gather per run.
...
/// server ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â used by the chunk-load benchmark as it ramps the
...
pub fn update_game(
    ...
    // entities/player, held item, clouds, or weather ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â and
    // skipping them also keeps the measured frame times honest.
...
/// ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no resource-pack support). 0 = none, 1 = partial, 2 =
/// full.
